...
void lStr_memmove(lChar32 * dst, const lChar8 * src, int count)
{
    _lStr_memcpy(dst, src, count);
}
...
template<typename T>
void lString32::_assign(const T * str, size_type len)
{
    if (pchunk->nref == 1)
    {
        if (pchunk->size < len)
...
lString32 & lString32::assign(const lChar32 * str, size_type count)
{
    ...
    {
        _assign(str, lStr_nlen(str, count));
    }
    return *this;
}
