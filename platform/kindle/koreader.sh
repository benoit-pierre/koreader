...
ko_update_check() {
    ...
    if [ -f "${NEWUPDATE}" ]; then
        ...
        FBINK_PID="$(/var/tmp/fbink --daemon 1 %KOREADER% -q -y -6)"
        eval "$(/var/tmp/fbink -e)"
        ...
        # Ditto with pv.
        cp -pf "${KOREADER_DIR}/bsdtar" "${KOREADER_DIR}/pv" /var/tmp/
        /var/tmp/pv --force --format=' %t %p %e ' --width="${MAXCOLS}" "${NEWUPDATE}" 2>"${FBINK_NAMED_PIPE}" |
            /var/tmp/bsdtar --no-same-permissions --no-same-owner -C "/mnt/us" -x
        fail=$?
        kill -TERM "${FBINK_PID}"
        # And remove our temporary binaries...
        rm -f /var/tmp/bsdtar /var/tmp/pv
        # Cleanup behind us...
        if [ "${fail}" -eq 0 ]; then
