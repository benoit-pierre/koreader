...
update: all
	...
	# create new package
	cd $(INSTALL_DIR) && zip -0 -r ../$(PB_PACKAGE) applications -x $(patsubst '%','applications/%',$(release_excludes))
	# generate pocketbook package index file
	zipinfo -1 $(PB_PACKAGE) >$(INSTALL_DIR)/applications/koreader/ota/package.index
	echo "applications/koreader/ota/package.index" >>$(INSTALL_DIR)/applications/koreader/ota/package.index
	# hack file path when running tar in parent directory of koreader
	sed -i -e 's/^/..\//' $(INSTALL_DIR)/applications/koreader/ota/package.index
	LC_ALL=C sort -o $(INSTALL_DIR)/applications/koreader/ota/package.index{,}
	# update index file in zip package
	cd $(INSTALL_DIR) && zip -ru ../$(PB_PACKAGE) applications/koreader/ota/package.index system
