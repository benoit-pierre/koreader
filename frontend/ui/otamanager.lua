...
local OTAManager = {
    ...
    kotasync_update_package = ota_dir .. "update.zip",
...
function OTAManager:_prepareUpdate()
    local progress
    local aborted = false
    local stats = self.updater:prepare_update(function(count, total)
...
function OTAManager:_fetchUpdate(download_size)
    ...
    local aborted = false
    local ok, err = self.updater:download_update_in_subprocess(function(size)
