...
# Exclude everything in `tools/`, except the Kobo touch probe script on Kobo targets.
ifeq (,$(KOBO))
  UPDATE_PATH_EXCLUDES += tools
else
  UPDATE_PATH_EXCLUDES += $(filter-out tools/kobo%,$(wildcard tools/*))
endif
