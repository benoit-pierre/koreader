...
kotasync_make() {
    ...
    if [[ -f "artifacts/old/${mfst}" ]]; then
        cmd+=(--reorder "artifacts/old/${mfst}")
    fi
    cmd+=("artifacts/new/${txz}" "artifacts/new/${mfst}")
    run "${cmd[@]}"
}
...
for a in artifacts/new/*; do
    a="${a##*/}"
    case "${a}" in
        koreader-kindlepw2-*.tar.xz) kotasync_make "${a}" koreader-kindlepw2-latest-nightly.kotasync ;;
    esac
done
