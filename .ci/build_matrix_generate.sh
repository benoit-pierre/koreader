...
mkdir -p base
cache_key_build_manifest="$(git -C base ls-files -z <"${CI_DIR}/build_cache_key_paths.txt" | xargs -0 git -C base ls-tree @)"
cache_key_build="$(sha256sum <<<"${cache_key_build_manifest}" | cut -d\  -f1)"
printf '%s\n' "${GITHUB_ACTIONS:+::group::}${ANSI_BLUE}build cache key: ${cache_key_build}${ANSI_RESET}" "${cache_key_build_manifest}" "${GITHUB_ACTIONS:+::endgroup::}"
...
printf '%s\n' "${GITHUB_ACTIONS:+::group::}${ANSI_BLUE}caches:${ANSI_RESET}"
jq . <<<"${caches}"
