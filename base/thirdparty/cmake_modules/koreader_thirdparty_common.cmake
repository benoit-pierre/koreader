...
function(append_binary_install_command CMD_LIST)
    ...
    foreach(SRC IN LISTS _UNPARSED_ARGUMENTS)
        ...
        if(APPLE)
            ...
            list(APPEND ${CMD_LIST} COMMAND ${OBJCOPY})
            if(KEEP_DEBUG)
                list(APPEND ${CMD_LIST} --add-gnu-debuglink=${DST}.dbg)
            endif()
            list(APPEND ${CMD_LIST} --strip-unneeded ${SRC} ${DST})
        endif()
...
function(find_compiler_lib_path VAR LIB)
    ...
    if(DEFINED CACHE{COMPILER_LIBRARIES_SEARCH_PATHS})
        ...
        ensure_execute_process(COMMAND ${CMAKE_C_COMPILER} -print-search-dirs OUTPUT_VARIABLE OUT)
        if(NOT OUT MATCHES "\nlibraries: =([^\n]+)\n")
...
function(find_executable VAR NAME BINARY VERSION_ARG)
    ...
    ensure_execute_process(COMMAND ${${VAR}} ${VERSION_ARG} OUTPUT_VARIABLE VERSION OUTPUT_STRIP_TRAILING_WHITESPACE)
    string(REGEX MATCH "(^|[ \n])([0-9][.0-9]*)([\n ]|$)" VERSION "${VERSION}")
...
function(ensure_execute_process)
    cmake_parse_arguments("" "" "INPUT_FILE;OUTPUT_FILE;OUTPUT_QUIET;OUTPUT_STRIP_TRAILING_WHITESPACE;OUTPUT_VARIABLE;WORKING_DIRECTORY" "COMMAND" ${ARGN})
    if(_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "unparsed arguments: ${_UNPARSED_ARGUMENTS}")
    endif()
    execute_process(${ARGN} RESULT_VARIABLE RET ERROR_VARIABLE ERR_OUT)
    if(NOT RET EQUAL 0)
        message(FATAL_ERROR "execution failed [${RET}]: ${_COMMAND}\n${ERR_OUT}")
    endif()
    if(_OUTPUT_VARIABLE)
