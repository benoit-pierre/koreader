...
#define IS_LSF()  (__BYTE_ORDER == __LITTLE_ENDIAN)

/// byte order convertor
struct lvByteOrderConv {
    /// reverse 64 bit word
    ...
    inline static lUInt64 lsf( lUInt64 w )
    {
        return IS_LSF() ? w : rev(w);
    }
    /// make 64 bit word most-significant-first byte order (PPC)
    inline static lUInt64 msf( lUInt64 w )
    {
        return !IS_LSF() ? w : rev(w);
    }
    /// make 32 bit word least-significant-first byte order (Intel)
    inline static lUInt32 lsf( lUInt32 w )
    {
        return IS_LSF() ? w : rev(w);
    }
    /// make 32 bit word most-significant-first byte order (PPC)
    inline static lUInt32 msf( lUInt32 w )
    {
        return !IS_LSF() ? w : rev(w);
    }
    /// make 16 bit word least-significant-first byte order (Intel)
    inline static lUInt16 lsf( lUInt16 w )
    {
        return IS_LSF() ? w : rev(w);
    }
    /// make 16 bit word most-significant-first byte order (PPC)
    inline static lUInt16 msf( lUInt16 w )
    {
        return !IS_LSF() ? w : rev(w);
    }
    inline static void rev( lUInt64 * w )
    {
        *w = rev(*w);
    }
    inline static void rev( lUInt32 * w )
    {
        *w = rev(*w);
    }
    inline static void rev( lUInt16 * w )
    {
        *w = rev(*w);
    }
    inline static void msf( lUInt64 * w )
    {
        if (IS_LSF())
            *w = rev(*w);
    }
    inline static void lsf( lUInt64 * w )
    {
        if (!IS_LSF())
            *w = rev(*w);
    }
    inline static void msf( lUInt64 * w, int len )
    {
        if (IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void lsf( lUInt64 * w, int len )
    {
        if (!IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void msf( lUInt32 * w )
    {
        if (IS_LSF())
            *w = rev(*w);
    }
    inline static void lsf( lUInt32 * w )
    {
        if (!IS_LSF())
            *w = rev(*w);
    }
    inline static void msf( lUInt32 * w, int len )
    {
        if (IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void lsf( lUInt32 * w, int len )
    {
        if (!IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void msf( lUInt16 * w, int len )
    {
        if (IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void lsf( lUInt16 * w, int len )
    {
        if (!IS_LSF()) {
            for ( int i=0; i<len; i++)
                w[i] = rev(w[i]);
    ...
    inline static void msf( lUInt16 * w )
    {
        if (IS_LSF())
            *w = rev(*w);
    }
    inline static void lsf( lUInt16 * w )
    {
        if (!IS_LSF())
            *w = rev(*w);
    }
    inline static bool lsf()
    {
        return IS_LSF();
    }
    inline static bool msf()
    {
        return !IS_LSF();
    }
};
