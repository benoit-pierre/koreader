...
LIBRX = {
    'android': re.compile(r'^lib(?P<name>[^.]+).so(?:.(?P<version>\d+[.0-9]*))?$'),
    'darwin': re.compile(r'^lib(?P<name>[^.]+)(?:.(?P<version>\d+[.0-9]*))?.dylib$'),
    'linux': re.compile(r'^lib(?P<name>[^.]+).so(?:.(?P<version>\d+[.0-9]*))?$'),
}
