...
function Updater:new(state_dir, seed, dl)
    local o = {}
    setmetatable(o, self)
    self.__index = self
...
function Updater:download_update_in_subprocess(progress_cb, progress_frequency)
    ...
    local child, read_fd = util.runInSubProcess(function(pid, write_fd)
        ...
        local send = function(...)
            buf:reset():encode{...}
            msg_len[0] = #buf
            posix.write(write_fd, msg_len, 2)
            local ptr = buf:ref()
            posix.write(write_fd, ptr, msg_len[0])
        end
    ...
    local buf = buffer:new()
    while true do
        posix.read(read_fd, msg_len, 2)
        local ptr = buf:reserve(msg_len[0])
        posix.read(read_fd, ptr, msg_len[0])
